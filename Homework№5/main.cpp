...
int main() {
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                if (event.mouseButton.button == sf::Mouse::Left) {
                    ...
                    float mathY = -(mousePos.y - 300) / 10.0f; // Масштаб 30 по Y
        ...
        drawGraph(window, [](float x) { return -5 * x + 3; }, -10, 10, 30, 10, sf::Color::Red);

        // 5 _ Отрисовка графика y2 = 3*x - 6
        drawGraph(window, [](float x) { return 3 * x - 6; }, -10, 10, 30, 10, sf::Color::Blue);
